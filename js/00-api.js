...
async function apiGet(){
  ...
  return {clients:(raw.clients||[]).map(mapClient),catalog:(raw.serviceCatalog||[]).map(mapCatalog),services:(raw.contracts||[]).map(mapContract),quotes:raw.quotes||[],settings:{documentType:'Recibo por Honorarios',retentionRate:8,noRetentionThreshold:1500,paymentHolder:'Claudia Romero Fonseca'}};
}
